...
def ensure_contacts_first_seen_batch(contact_ids: list[int], timestamp: float):
    """Record first-seen time for multiple contacts in a single transaction (INSERT OR IGNORE)."""
    if not contact_ids:
        return
    rows = [(cid, timestamp) for cid in contact_ids]
    with _writer_transaction() as conn:
        cursor = conn.cursor()
        cursor.executemany(
            "INSERT OR IGNORE INTO contact_first_seen (contact_id, first_seen_at) VALUES (?, ?)",
