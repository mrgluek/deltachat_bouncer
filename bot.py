...
def _cmping_monitor_cycle(bot, accid, cmping_path):
    ...
    if not any_source_success and len(targets) >= 2:
        ...
        for dst in targets:
            ...
            old_dst_healthy = _cmping_server_status.get(dst, True)
...
@dc_cli.on(events.NewMessage(command="/cmpingevents"))
@dc_cli.on(events.NewMessage(command="/cmpingincidents"))
@dc_cli.on(events.NewMessage(command="/cmevents"))
...
@dc_cli.on(events.NewMessage(command="/cmhistory"))
def cmpinghistory_command(bot, accid, event):
    ...
    if not server_arg:
        if not all_servers:
            _send(bot, accid, msg.chat_id, "ℹ️ No servers currently monitored.")
            return
        ...
        for srv in all_servers:
            is_healthy = _cmping_server_status.get(srv, True)
    ...
    is_healthy = _cmping_server_status.get(target_server, True)
    icon = "🟢" if is_healthy else "🔴"
    status_txt = "HEALTHY" if is_healthy else "UNHEALTHY"
    ...
    if not events_list:
        ...
        for ev in events_list:
            ...
            if went_up:
                ...
                duration_str = _format_duration(max(1, went_up - went_down))
                lines.append(f"• `{down_str}` → `{up_str} UTC` (`{duration_str}`) — `{error_reason}`")
            else:
                duration_str = _format_duration(max(1, int(time.time()) - went_down))
...
def _vt_wait_rate_limit():
    ...
    global _vt_last_request_time
    now = time.time()
    elapsed = now - _vt_last_request_time
    if elapsed < VIRUSTOTAL_RATE_LIMIT_SECONDS:
        time.sleep(VIRUSTOTAL_RATE_LIMIT_SECONDS - elapsed)
    _vt_last_request_time = time.time()
...
def _vt_api_request(method: str, endpoint: str, vt_key: str, data: dict | None = None) -> tuple[dict | None, int, str | None]:
    ...
    headers = {
        "x-apikey": vt_key,
        "User-Agent": f"BouncerBot/{VERSION}",
...
def bg_virus_worker(bot, accid, chat_id, msg_id, target_type, target_data, vt_key):
    ...
    try:
        ...
        _react(bot, accid, msg_id, "❌")
        _send(bot, accid, chat_id, f"❌ An error occurred during the VirusTotal check: {e}", reply_to_id=msg_id)
    finally:
        _vt_global_lock.release()


@dc_cli.on(events.NewMessage(command="/virus"))
def virus_command(bot, accid, event):
    ...
    if payload:
        url = _extract_first_url(payload)
        if url:
            target_type = "url"
            target_data = url
